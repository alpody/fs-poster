...
trait Ajax
{
    ...
    function reset_custom_settings ()
    {
        ...
        Helper::setCustomSetting( 'unique_link', Helper::getOption( 'unique_link', '1' ), $node_type, $node_id );
        Helper::setCustomSetting( 'url_shortener', Helper::getOption( 'url_shortener', '0' ), $node_type, $node_id );
        Helper::setCustomSetting( 'shortener_service', Helper::getOption( 'shortener_service' ), $node_type, $node_id );
        Helper::setCustomSetting( 'url_short_access_token_bitly', Helper::getOption( 'url_short_access_token_bitly', '100' ), $node_type, $node_id );
        Helper::setCustomSetting( 'url_additional', Helper::getOption( 'url_additional', '' ), $node_type, $node_id );
        Helper::setCustomSetting( 'share_custom_url', Helper::getOption( 'share_custom_url', '0' ), $node_type, $node_id );
        Helper::setCustomSetting( 'custom_url_to_share', Helper::getOption( 'custom_url_to_share', '{site_url}/?p={post_id}&feed_id={feed_id}' ), $node_type, $node_id );
	...
	public function create_account_group ()
	{
		...
		if ( empty( $name ) )
		{
			Helper::response( FALSE, 'Name can\'t be empty' );
		}
	...
	function remove_from_group ()
	{
		...
		if ( empty( $node_id ) || empty( $node_type ) || empty( $group_id ) )
		{
			Helper::response(FALSE);
		}
