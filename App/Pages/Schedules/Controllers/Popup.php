...
trait Popup
{
	...
	public function add_schedule ()
	{
		...
		{
			if ( ! in_array( $post_type, $allowedPostTypes ) || $post_type === 'attachment' )
			{
				continue;
			}
		...
		if ( ! empty( $post_ids ) )
		{
			...
			if ( $post_ids_count == 1 )
			{
				...
				$schedule_name = ! empty( $onePostInf[ 'post_title' ] ) ? 'Scheduled post: "' . Helper::cutText( $onePostInf[ 'post_title' ] ) . '"' : '';
			}
			else
			{
