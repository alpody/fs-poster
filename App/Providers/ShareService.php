...
class ShareService
{
	...
	public static function scheduledPost ( $schedule_info )
	{
		...
		if ( ! empty( $nodes_list ) )
		{
			self::insertFeeds( $post_id, $nodes_list, $customPostMessages, FALSE, NULL, 1, $scheduleId, TRUE );
	...
	public static function post ( $feedId, $secureShare = FALSE )
	{
		...
		if ( $driver == 'fb' )
		{
			...
			if ( empty( $options ) ) // App method
			{
				...
				else
				{
					...
					$res = Facebook::sendPost( $nodeProfileId, $sendType, $message, 0, $link, $images, $videoURL, $accessToken, $proxy, $poster_id );
				}
		...
		else if ( $driver === 'plurk' )
		{
			...
			if ( $sendType != 'image' && $sendType != 'video' )
			{
				$pMethod = Helper::getOption( 'plurk_posting_type', '1' );
		...
		else if ( $driver == 'pinterest' )
		{
			...
			else // Cookie method
			{
				if ( $sendType != 'image' )
				{
					$thumbPath = WPPostThumbnail::getPostThumbnail( $post_id, $driver );
		...
		else if ( $driver == 'reddit' )
		{
			...
			$res = Reddit::sendPost( $node_info[ 'info' ], $sendType, $postTitle, $message, $longLink, $images, $videoURL, $accessToken, $proxy );
		}
		else if ( $driver == 'tumblr' )
		{
			...
			$res = Tumblr::sendPost( $node_info[ 'info' ], $sendType, $postTitle, $message, $link, $imagesLocale, $videoURLLocale, $accessToken, $accessTokenSecret, $appId, $proxy, $tags );
		}
		...
		else if ( $driver == 'google_b' )
		{
			if ( empty( $options ) )
			{
				...
				if ( $sendType != 'image' && $sendType != 'video' )
				{
					$post_method = Helper::getOption( 'google_b_posting_type', '1' );
		...
		{
			$page_as_page       = Helper::getOption( 'blogger_posting_type', '0' ) == '1';
			...
			{
				$message = "<style>[class^='wp-image']{width:100%;height:auto;}</style>" . $message;
			}
		...
		else
		{
			...
			$udpateDate = [
				...
				'feed_type'       => isset( $res[ 'feed_type' ] ) ? $res[ 'feed_type' ] : NULL
			];
